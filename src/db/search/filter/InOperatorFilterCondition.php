...
class InOperatorFilterCondition extends BaseOperatorFilterCondition
{
    ...
    public function rules()
    {
        return [
            ...
            ['valueType', 'in', 'range' => [SearchModel::SEARCH_INT_ARRAY, SearchModel::SEARCH_LITERAL_ARRAY]],
            ['valueType', 'required'],
    ...
    {
        $filter = '';
        $padding = ($this->valueType == SearchModel::SEARCH_INT_ARRAY) ? '' : '"';
    ...
    {
        if ($this->valueType == SearchModel::SEARCH_INT_ARRAY) {
            $value = intval($value);
        }
        return $value;
    }
